...
INSERT INTO @fnmaOsar (MD_STATEMENT_ID, MD_NOI_CAT_TYPE_ORDER, MD_NOI_CAT_ORDER, MD_NOI_CAT_TYPE_NAME, STATEMENT_YEAR, NOI_CATEGORY, REPORTED, NORMALIZED) 
	...
	(SELECT OpStatementHeaderId_F, 30, MAX(b.OrderKey) + 10 ,'Capital Expenditures', StatementYear, 'Total Capital', SUM(ItemAmount), SUM(ItemAmountAfterAdjustment) FROM tblOpStatementDetail 
		...
		GROUP BY OpStatementHeaderId_F, StatementYear)
	UNION
	(SELECT OpStatementHeaderId_F, 35, 9960,'', StatementYear, 'Total Expense & Replacement Reserve', SUM(ItemAmount), SUM(ItemAmountAfterAdjustment) FROM tblOpStatementDetail 
		INNER JOIN tblOpStatementHeader ON OpStatementHeaderId = OpStatementHeaderId_F
		INNER JOIN tblzCdNOICategory b ON NOICategoryCd = NOICategoryCd_F
		WHERE OpStatementHeaderId_F IN  (SELECT MD_STMT_ID FROM @OpStmts)
...
INSERT INTO @fnmaOsar (LOANNUMBER, MD_STATEMENT_ORDER, MD_NOI_CAT_TYPE_ORDER, MD_NOI_CAT_TYPE_NAME, MD_STATEMENT_ID, MD_NOI_CAT_ORDER, STATEMENT_YEAR, NOI_CATEGORY, REPORTED, ADJUSTMENTS, NORMALIZED) 
	SELECT DISTINCT a.LOANNUMBER, b.MD_STMT_ORDER, 40, 'Cash Flow Analysis', b.MD_STMT_ID, 10, b.STMT_NAME, 'Debt Service - All Notes ', 0, 0, @debtService FROM @fnmaOsar a INNER JOIN @OpStmts b ON a.LOANNUMBER = b.LOAN_NUMBER
...
INSERT INTO @fnmaOsar (LOANNUMBER, MD_STATEMENT_ORDER, MD_NOI_CAT_TYPE_ORDER, MD_NOI_CAT_TYPE_NAME, MD_STATEMENT_ID, MD_NOI_CAT_ORDER, STATEMENT_YEAR, NOI_CATEGORY, REPORTED, NORMALIZED) 
	SELECT 
		LOANNUMBER, MD_STATEMENT_ORDER, 35 AS MD_NOI_CAT_TYPE_ORDER, '' AS MD_NOI_CAT_TYPE_NAME , MD_STATEMENT_ID, 9950 AS MD_NOI_CAT_ORDER, STATEMENT_YEAR,
	...
	UNION
	SELECT 
		LOANNUMBER, MD_STATEMENT_ORDER, 35, '' , MD_STATEMENT_ID, 9970, STATEMENT_YEAR,
		'Net Cash Flow',
